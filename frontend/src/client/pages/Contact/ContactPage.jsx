...
export default function ContactPage() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setError(submitError.response?.data?.message || t('contact.errorSubmit'));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const contactRows = [
    { icon: <MapPin size={20} strokeWidth={1.5} />, text: mainBranch?.address || '123 Nguyễn Huệ, Quận 1, TP. Hồ Chí Minh' },
    { icon: <Phone size={20} strokeWidth={1.5} />, text: mainBranch?.phone || '0123456789' },
    ...
    <div style={{ paddingTop: '72px', paddingBottom: 'var(--sp-80)', background: 'var(--c-surface)', minHeight: '100vh' }}>
      <div style={{ background: 'linear-gradient(135deg,var(--c-primary),var(--c-primary-container))', padding: 'var(--sp-64) var(--sp-24)' }}>
        <div className="container">
          <h1 className="display-md" style={{ color: 'var(--c-on-primary)', marginBottom: 'var(--sp-12)' }}>{t('contact.heroTitle')}</h1>
          <p className="body-lg" style={{ color: 'rgba(255,255,255,0.82)' }}>{t('contact.heroSubtitle')}</p>
        </div>
      </div>
        ...
        <div className="container">
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.5fr', gap: 'var(--sp-48)', alignItems: 'start' }}>
            <div>
              ...
              <div style={{ marginTop: 'var(--sp-32)', borderRadius: 'var(--r-2xl)', overflow: 'hidden', height: 280, background: 'var(--c-surface-container)', boxShadow: '0 4px 24px rgba(0,0,0,0.10)' }}>
                <iframe
                  title="Hotel location map"
                  src={
                    mainBranch?.mapEmbedLink ||
                    'https://maps.google.com/maps?q=123+Nguy%E1%BB%85n+Hu%E1%BB%87%2C+Qu%E1%BA%ADn+1%2C+TP.HCM&output=embed&hl=vi'
                  }
              ...
              <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 'var(--sp-20)' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--sp-20)' }}>
                  <div className="input-tray">
                    <label htmlFor="ct-name">{t('contact.labelName')} *</label>
                ...
                <button type="submit" className="btn btn-primary" id="contact-submit-btn" disabled={submitting}>
                  <Send size={16} /> {submitting ? t('contact.submitting') : t('contact.submitBtn')}
                </button>
              </form>
            </div>
          </div>
